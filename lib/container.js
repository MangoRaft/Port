...
Container.prototype._detectPort = function(callback) {
	...
	this.inspect(function(err, inspect) {
		if (err) {
			var error = new Error('Container._detectPort.inspect');
			error.err = err;
			self.emit('error', error);
			return callback(error);
		}
...
Container.prototype.inspect = function(callback) {
	debug('Container.inspect Calling inspect on ' + this.id)
	var self = this;
	this.container.inspect(function(err, data) {
		if (err) {
			var error = new Error('Container._inspect.inspect');
			error.err = err;
			self.emit('error', error);
			return callback(error);
		}
...
Container.prototype._start = function(callback) {
	...
	this.container.start(function(err, data) {
		if (err) {
			var error = new Error('Container._start.start');
			error.err = err;
			self.emit('error', error);
			return callback(error);
		}
