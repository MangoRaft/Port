...
Container.prototype.exec = function(options, callback) {
	var self = this;

	options.container = self.container;
	options.logs = self.options.logs;
...
Container.prototype._clean = function(callback) {
	...
	this.container.remove({
		...
		if (self.config.Image == 'registry.system.nodetopia.com/cedar:runner') {
			return callback();
		}
...
Container.prototype._wait = function() {
	...
	this.container.wait(function(error, data) {
		...
		console.log(self.shortLived, data.StatusCode == 0)
