...
Port.prototype.start = function(options, cb) {
	...
	var _options = {
		...
		metricSesion : options.metricSesion,
		logs : options.logs,
		...
		config : {
			...
			"Memory" : options.limits.memory || 0,
			"MemorySwap" : options.limits.swap || 0,
			"CpuShares" : options.limits.cpuShares || 512,
			"Cpuset" : options.limits.cpuset || "0,1",
			"Tty" : false,
			"Image" : options.image,
			"Volumes" : {
...
Port.prototype.stop = function(uid, cb) {
	...
	if (container.stopping || container.deid) {
		return cb();
	}
...
Port.prototype._start = function(options, cb) {
	...
	container.on('start', function() {
		container.info(function(err, info) {
			self.emit('start', info);
			cb(null, info);
		});
		self.containers[container.id] = container;
	});

	container.on('stop', function(data) {
		delete self.containers[container.id];
		self.emit('stop', self._options(container.options));
	});
	container.on('death', function(data) {
		delete self.containers[container.id];
		self.emit('death', self._options(container.options));
	});
