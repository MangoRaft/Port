...
class Port extends events.EventEmitter {
    ...
    start(options) {
        ...
        if (_options.stats)
            _options.stats.key = options.metricSession + '.' + _options.config.Hostname
    ...
    avalibale() {
        return {
            memory: {
                ...
                free: os.freemem() / Math.pow(1024, 2)
            },
            cores: {
                count: this.cores,
                used: this.coresUsed,
