...
class Port extends events.EventEmitter {
    ...
    start(options) {
        ...
        if (options.env) {
            let env = new Volume(options.env, this);
            _options.env = env;
            env.add(_options.config)
        }
    ...
    avalibaleCPU() {
        return (this.cores * 1000) - this.coresUsed.reduce((accumulator, currentValue) => accumulator + currentValue)
    }
