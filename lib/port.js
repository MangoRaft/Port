...
Port.prototype.start = function(options, callback) {
	...
	if (options.env) {
		...
		_options.config.Env.push('MEMORY_AVAILABLE=' + (_options.config.Memory / 1048576));
		_options.config.Env.push('WEB_MEMORY=' + ((_options.config.Memory / ( typeof _options.config.Cpuset == 'number' ? _options.config.Cpuset : _options.config.Cpuset.split(',').length)) / 1048576));
		_options.config.Env.push('WEB_CONCURRENCY=' + ( typeof _options.config.Cpuset == 'number' ? _options.config.Cpuset : _options.config.Cpuset.split(',').length));
	}
	if (options.volumes) {
...
Port.prototype.addResources = function(memory, cpuset, cpuShares) {
	...
	for (var i = 0; i < cores; i++) {
		var c = Math.floor(Math.random() * (this.coresUsed.length));
		_cpuset.push(c);
		self.coresUsed[c] += cpuShares;

	}
	var c = _cpuset.join();
	console.log(c, self.coresUsed, this.usagedMemory);
	return c;
};
...
Port.prototype._start = function(options, callback) {
	...
	container.states.forEach(function(state) {
		container.on(state, function(data) {
			console.log('State changed to: ' + states[state] + '	' + options.config.name);
			if (state !== 'STARTING')
